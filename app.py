...
def add_debt_route():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO debts (debt_id, client_name, description, amount, phone_number)
            VALUES (%s, %s, %s, %s, %s)
        """, (debt_id, name, description, amount, phone))
        conn.commit()
    except Exception as e:
...
def delete_client():
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM debts WHERE client_name = %s AND paid = FALSE", (client_name,))
...
def redeem_debt_route(debt_id):
    try:
        ...
        # Update the debt to mark it as paid
        cur.execute("""
            UPDATE debts
            SET paid = TRUE
            WHERE debt_id = %s
        ...
        if cur.rowcount == 0:
            # No row was updated, debt_id does not exist
            conn.rollback()
            cur.close()
            conn.close()
        ...
        print(f"Error redeeming debt: {e}")
...
def clients():
    try:
        ...
        cur.execute("""
            SELECT client_name,
                BOOL_OR(paid = FALSE) AS has_unpaid,
                COALESCE(SUM(CASE WHEN paid = FALSE THEN amount ELSE 0 END), 0) AS total_unpaid
            FROM debts
            GROUP BY client_name
            ORDER BY client_name

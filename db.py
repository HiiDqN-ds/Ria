...
def load_debts():
    # Example using your DB connection (adjust to your setup)
    query = "SELECT * FROM debts WHERE paid = FALSE;"
    debts = fetch_all(query)  # You need a fetch_all function to execute SQL and return results as list of dicts
    return debts

def get_unpaid_sum():
    query = "SELECT SUM(amount) AS total FROM debts WHERE paid = FALSE;"
    result = fetch_one(query)  # fetch_one returns dict with 'total'
...
def calculate_purchases_for_date(date):
    ...
    for purchase in purchases:
        p_date = purchase.get('order_date') or purchase.get('date')
        if isinstance(p_date, str):
            try:
                p_date = datetime.fromisoformat(p_date).date()
            except Exception:
                continue
        if p_date == date:
            total += float(purchase.get('total_price', 0))
    return round(total, 2)
...
def save_kasse_balance_for_date(date, balance):
    try:
        query = """
            ...
            VALUES (%s, %s)
            ON CONFLICT (date) DO UPDATE SET closing_balance = EXCLUDED.closing_balance;
        """
        execute_query(query, (date, balance))
